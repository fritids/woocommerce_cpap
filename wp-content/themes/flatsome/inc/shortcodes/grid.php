...
function rowShortcode($atts, $content = null) {
  ...
  if($delay) {
	...
	$container = '<div class="row container '.$scroll.'" '.$scroll_html.' '.$delay_html.'>'.$content.'</div>';
	return $container;
} 
...
function colShortcode($atts, $content = null) {	
  ...
  if($delay) {
	...
	$content = do_shortcode($content);
	$column = '<div class="large-'.$span.' columns '.$scroll.'" '.$scroll_html.' '.$delay_html.'>'.$content.'</div>';
	return $column;
}
